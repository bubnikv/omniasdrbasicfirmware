...
*
*******************************************************************************/
void FRAC_CLK_Start(void) 
{
    /* Set the bit to enable the clock. */
    FRAC_CLK_CLKEN |= FRAC_CLK_CLKEN_MASK;
	FRAC_CLK_CLKSTBY |= FRAC_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: FRAC_CLK_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    FRAC_CLK_CLKEN &= (uint8)(~FRAC_CLK_CLKEN_MASK);
	FRAC_CLK_CLKSTBY &= (uint8)(~FRAC_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: FRAC_CLK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_StopBlock(void) 
{
    if ((FRAC_CLK_CLKEN & FRAC_CLK_CLKEN_MASK) != 0u)
    {
...
#if defined(FRAC_CLK__CFG3)
        CLK_DIST_AMASK = FRAC_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = FRAC_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* FRAC_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(FRAC_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(FRAC_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: FRAC_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        FRAC_CLK_CLKSTBY &= (uint8)(~FRAC_CLK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: FRAC_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = FRAC_CLK_GetSourceRegister();
    uint16 oldDivider = FRAC_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = FRAC_CLK_CLKEN & FRAC_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((FRAC_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: FRAC_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 FRAC_CLK_GetDividerRegister(void) 
{
    return CY_GET_REG16(FRAC_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: FRAC_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_SetModeRegister(uint8 modeBitMask) 
{
    FRAC_CLK_MOD_SRC |= modeBitMask & (uint8)FRAC_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: FRAC_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_ClearModeRegister(uint8 modeBitMask) 
{
    FRAC_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(FRAC_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: FRAC_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 FRAC_CLK_GetModeRegister(void) 
{
    return FRAC_CLK_MOD_SRC & (uint8)(FRAC_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: FRAC_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = FRAC_CLK_GetDividerRegister();
    uint8 oldSrc = FRAC_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        FRAC_CLK_MOD_SRC |= CYCLK_SSS;
        FRAC_CLK_MOD_SRC =
            (FRAC_CLK_MOD_SRC & (uint8)(~FRAC_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: FRAC_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 FRAC_CLK_GetSourceRegister(void) 
{
    return FRAC_CLK_MOD_SRC & FRAC_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: FRAC_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void FRAC_CLK_SetPhaseRegister(uint8 clkPhase) 
{
    FRAC_CLK_PHASE = clkPhase & FRAC_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: FRAC_CLK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 FRAC_CLK_GetPhaseRegister(void) 
{
    return FRAC_CLK_PHASE & FRAC_CLK_PHASE_MASK;
}
